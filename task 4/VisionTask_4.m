...
[m,n] = size(IRed);
%Color of cylinder(cyan(R=255,B=255,G=0)) used to set intensities of new image
%If pixel(x,y).color ~ cyan --> cylinder image pixel(x,y)-->255, else -->0
...
%Axes of ellipse which inscriebs the shape giving orientation information
...
%Laplacian edge detection using convolution mask [[-1 -1 -1];[-1 8 -1];[-1 -1 -1]]
...
CornerBox % returns a value of 6 becasue of the internal corners given by the cross pattern
          % Enough to distinguish cylinder from cube
